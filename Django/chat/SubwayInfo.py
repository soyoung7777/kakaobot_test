...
def detail_get_subway_station_and_number_information(subwayData):
    ...
    for idx, full_list in enumerate(direction_stationlist):
        text +="💌["+stationName+" "+current_laneName+" "+direction[idx]+"]💌<br/><br/><br/>"
        for s in full_list:
            ...
            for element in StationExistNameList:
                if eq(element,total):
                    if eq(total,full_list[6]):
                        text+="<font color='#DC6D73'style='font-weight: bold;'>"+total+"</font>🚋<br/><br/>"
                    else:
                        text+="<font color='#DC6D73'style='font-weight: bold;'>"+total+"</font>🚋<br/><br/>   ↓↓↓   <br/><br/>"
                    exist = True
            if exist==False:
                # if eq(total,"none"):
